...
type FakeRace = { season: number; slug?: string };

/**
 * Stands in for the two index reads `getCurrentSeason` makes: upcoming races
 * by lock time, and the most recent races by start time.
 */
function fakeCtx(rows: {
  upcoming?: Array<FakeRace>;
  latest?: Array<FakeRace>;
}): Pick<QueryCtx, 'db'> {
  const db = {
    query: () => ({
...
describe('getCurrentSeason', () => {
  ...
  it('skips leftover e2e fixtures that lock before the real GP', async () => {
    const season = await getCurrentSeason(
      fakeCtx({
        upcoming: [
          {
          ...
          { season: 2026, slug: 'madrid-2026' },
        ],
        latest: [{ season: 2026, slug: 'italy-2026' }],
      }),
    );
  ...
  it('rolls over once the last race of a season has run', async () => {
    ...
    const season = await getCurrentSeason(
      fakeCtx({ upcoming: [], latest: [{ season: 2027 }] }),
    );
  ...
  it('keeps a finished season showing itself, not an empty next one', async () => {
    const season = await getCurrentSeason(
      fakeCtx({ upcoming: [], latest: [{ season: 2026 }] }),
    );
