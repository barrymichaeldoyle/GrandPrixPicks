...
export const BAKU_DATASET_INCIDENT_COUNT = 58;
...
export function bakuCrashDatasetSchema(pagePath: string) {
  return {
    ...
    description: `Every recorded time a car hit a wall, a barrier, another car or something on the track at the Baku City Circuit, across the nine Formula 1 weekends held there. ${BAKU_DATASET_INCIDENT_COUNT} incidents involving ${BAKU_DATASET_DRIVER_COUNT} drivers, each with the session, the corner where the car stopped, and a citation. Curated rather than exhaustive: race control names a turn for car-to-car collisions but logs a solo wall hit as a bare red flag, so the set was researched per weekend.`,
    url: `${siteConfig.url}${pagePath}`,
    license: 'https://creativecommons.org/licenses/by/4.0/',
    isAccessibleForFree: true,
    /*
     * The whole Organization node, not a bare `@id` reference. This page does
     * not emit `#organization` anywhere else, and Google's Dataset validator
     * does not follow references across pages: on 19 September 2026 Search
     * Console flagged the bare reference as "Invalid object type for field
     * creator". Embedding `organizationSchema()` itself keeps the `@id`, so it
     * is still the same entity and the same copy the SEO invariants compare.
     */
