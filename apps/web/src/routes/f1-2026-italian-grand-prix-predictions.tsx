...
export const Route = createFileRoute('/f1-2026-italian-grand-prix-predictions')(
  {
    ...
    head: ({ loaderData }) =>
      raceWriteupPageHead({
        ...
        description: {
          live: '2026 Italian Grand Prix predictions at Monza. Pick a top 5 for qualifying and the race, with the form, tyre choice and driver news that decide a Monza result.',
          finished:
...
function TrackMap() {
  return (
    <RaceWriteupSection
      id="track-map"
      heading="Where overtakes happen"
      extra={
        <div className="mt-7 grid gap-7 lg:grid-cols-[minmax(0,1.45fr)_minmax(18rem,0.9fr)] lg:items-start">
          ...
          <div className="border-t border-border pt-6 lg:border-t-0 lg:border-l lg:pt-0 lg:pl-7">
            <p className="gpp-reading-copy text-text-muted">
              ...
              sits just before it, so a driver who gets through there close
              behind keeps the tow onto the main straight and can use Overtake
              down it, into Rettifilo. That is where most passes happen.
            </p>
            {/* The one line the deleted "Before you lock your Top 5" section
                carried that nothing else on the page did: what the geography
                above means for the order of a pick. It belongs here, beside
                the corner it is about, rather than in a list of method steps
                that restated the tow, the long runs and the practice sessions
                the sections above had already covered. */}
            <p className="gpp-reading-copy mt-3 text-text-muted">
              Rettifilo compresses the field into one heavy stop on lap one, so
              a driver at the back of your Top 5 carries more opening-lap risk
              here than at most circuits.
            </p>
            {/* The drawn lap and the real thing, in one column. The map is the
                ...
                way down it and left the rest of the column empty; a photograph
                of the kerbs the map draws as lines is the one picture that
                belongs next to it. Landscape, unlike every other photo on the
                page: a portrait here would stand taller than the map it is
                explaining. */}
    ...
    >
      ...
      <p className="gpp-reading-copy mt-3 text-text-muted">
        The kerbs at Turns 1, 4 and 8 have been rebuilt for 2026, and the gravel
        on the exit of Roggia is now asphalt run-off.{' '}
  ...
  return (
    <section className="py-8 sm:py-16" aria-labelledby="monza-seats">
      ...
      {/* The same two columns every other photo section on this page uses, so
          ...
          copy \u2014 because it had. The heading stays outside the grid
          because the card is the section, not a block of copy with a photo
          beside it. */}
      <div className="mt-7">
        <RaceWriteupPhotoLayout
          photo={<WriteUpNewsPhoto {...HADJAR_WRITEUP_IMAGE} />}
        >
          {/* Three across from `sm`, not three rows. Each row is a label, a
              ...
              actually making \u2014 one seat empty, one filled, one untouched
              \u2014 and they fill the column rather than trailing off half
              way. */}
...
function McLarenForm() {
  return (
    <RaceWriteupPhotoSection
      id="mclaren-form"
      heading="Monza is a different test for McLaren"
      mirrored
      teamColour={TEAM_COLORS.McLaren ?? FALLBACK_TEAM_COLOR}
      photo={<WriteUpNewsPhoto {...MCLAREN_PAIR_WRITEUP_IMAGE} />}
    >
      <p className="gpp-reading-copy mt-4 text-text-muted">
        McLaren won in Hungary and Zandvoort, both high-downforce races. Andrea
        Stella says the MCL40 has been weaker on drag and braking. Monza tests
        both, and Friday is where it shows in Norris and Piastri&rsquo;s
        straight-line speed.{' '}
...
function TyreChoice() {
  return (
    <RaceWriteupPhotoSection
      id="tyre-choice"
      heading="Monza gets the three softest tyres"
      mirrored
      photo={<WriteUpNewsPhoto {...PIRELLI_COMPOUND_WRITEUP_IMAGE} />}
    >
      ...
      <p className="gpp-reading-copy mt-3 text-text-muted">
        A stop at Monza costs more time than at almost any other race, so teams
        will try to one-stop. Heat is the usual reason that fails. Pirelli
        expects the rears to overheat out of the chicanes rather than wear out,
        which keeps a one-stop on.
      </p>
      ...
      <p className="gpp-reading-copy mt-3 text-text-muted">
        A one-stop puts the weight on qualifying and track position. A second
        stop favours the drivers who look after their tyres over the ones who
        are only quick over a single lap.{' '}
...
function HeatHazard({ showPickRead }: { showPickRead: boolean }) {
  return (
    <RaceWriteupPhotoSection
      ...
      photo={<WriteUpNewsPhoto {...MONZA_HEAT_WRITEUP_IMAGE} />}
    >
      <p className="gpp-reading-copy mt-4 text-text-muted">
        The official forecast put the heat index above 31°C for the race, which
        is the threshold in Article B1.5.10. Austria in June was the
        season&rsquo;s first heat hazard.
      </p>
      <p className="gpp-reading-copy mt-3 text-text-muted">
        A heat hazard makes the driver cooling system mandatory and raises the
        minimum weight by 5kg to cover the kit and its battery. Wearing the vest
        is still the driver&rsquo;s choice, and anyone who leaves it off carries
        0.5kg of ballast instead.{' '}
...
function SafetyCarLivery() {
  return (
    <RaceWriteupSection
      ...
      heading="The safety car runs in Lightning McQueen colours"
    >
      <p className="gpp-reading-copy mt-4 text-text-muted">
        Formula 1 and Disney have put a Cars livery on the safety car for Monza,
        twenty years after the film. A full-size Lightning McQueen, badged as a
        Formula 1 track vehicle, laps the circuit over the weekend. It is the
        same safety car at the same speeds, so a deployment costs the field what
        it always does.{' '}
...
function FerrariTribute() {
  return (
    ...
    // same 3px bar the driver badges use rather than a fourth kind of accent.
    // Ferrari red is read from the tokens, not typed in here.
    <RaceWriteupPhotoSection
      id="ferrari-tribute"
      heading="Ferrari runs a Schumacher tribute"
      teamColour={TEAM_COLORS.Ferrari ?? FALLBACK_TEAM_COLOR}
      photo={<WriteUpNewsPhoto {...SCHUMACHER_TRIBUTE_WRITEUP_IMAGE} />}
    >
      <p className="gpp-reading-copy mt-4 text-text-muted">
        Ferrari has revealed a one-off SF-26 livery for Monza, thirty years
        after Schumacher&rsquo;s first season in red. The car is extra red, with
        the white gone from the engine cover, retro driver numbers,
        Schumacher&rsquo;s signature on the cover, gold on the BBS rims, and his
        seven stars on the nose. Barrichello and Vettel will take the F2002
        around on Saturday and Sunday.
      </p>
      <p className="gpp-reading-copy mt-3 text-text-muted">
        Hamilton and Leclerc&rsquo;s race suits are out: red with white stripes,
        and seven stars on the back for Schumacher&rsquo;s titles.{' '}
        <ExternalSource href={SUITS_SOURCE}>See the race suits</ExternalSource>.{' '}
...
function NorrisContract() {
  return (
    <RaceWriteupPhotoSection
      id="norris-contract"
      heading="Norris re-signs with McLaren to 2030"
      mirrored
      teamColour={TEAM_COLORS.McLaren ?? FALLBACK_TEAM_COLOR}
      photo={<WriteUpNewsPhoto {...NORRIS_WRITEUP_IMAGE} />}
    >
      <p className="gpp-reading-copy mt-4 text-text-muted">
        McLaren has confirmed a new deal keeping Lando Norris at the team until
        at least the end of 2030, with a multi-year option beyond that. He
        joined as a test and development driver in 2017 and has raced for them
        since 2019. Oscar Piastri is contracted to the end of 2028.{' '}
...
function ColapintoContract() {
  return (
    <RaceWriteupPhotoSection
      ...
      heading="Colapinto stays at Alpine for 2027"
      teamColour={TEAM_COLORS.Alpine ?? FALLBACK_TEAM_COLOR}
      photo={<WriteUpNewsPhoto {...COLAPINTO_WRITEUP_IMAGE} />}
    >
      <p className="gpp-reading-copy mt-4 text-text-muted">
