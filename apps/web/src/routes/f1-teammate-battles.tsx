...
export const Route = createFileRoute('/f1-teammate-battles')({
  ...
  head: ({ loaderData }) => {
    ...
    const description = biggestGap
      ? `Who is beating their teammate in ${SEASON} F1? ${biggestGap.drivers[0].displayName} leads ${biggestGap.drivers[1].displayName} ${biggestGap.drivers[0].total}-${biggestGap.drivers[1].total}. Qualifying, sprint and race records for every pairing.`
      : `Head-to-head records for every ${SEASON} Formula 1 teammate pairing, split by qualifying, sprint and race from the official classification.`;
