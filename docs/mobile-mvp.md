...
- Personalized feed, including locked picks, published scores, and streaks.
...
- Support results, amended results, and session locked notifications.
- Push deep links:
...
- Toggles for prediction reminders, lock-soon reminders, results, and session locked.
...
| Feed                 | `feed.getPersonalizedFeed`, `feed.getFeedEvent`                                                                              |
...
events reuse web's exact names), users are
...
- feed loaded, paginated, and event opened;
...
- Feed pagination and empty-state discovery work for an account with no follows and an account with follows.
...
4. Feed: pagination, grouped session events, and event detail.
